...
num_reads_30x = num_reads(genomesize, readlength, coverage30x)
num_reads_30x = int(num_reads_30x)

## use an array to keep track of the coverage at each position in the genome
genome_coverage = np.zeros((genomesize), dtype=int)
for i in range(num_reads_30x): #trying to find where the reads come from in the 1Mbp genome
...
poisson_estimates = poisson.pmf(xs, mu = coverage30x)

## Get normal pdf at each of these (i.e. the density between each adjacent pair of points)
normal_estimates = norm.pdf(xs, loc = coverage30x, scale = np.sqrt(coverage30x))
...
with open("poisson_30x.txt", "w") as f:
    for x, count in zip(xs, poisson_counts):
       f.write(f"{x}\t{count}\n")

with open("normal_30x.txt", "w") as f:
    for x, count in zip(xs, normal_counts):
       f.write(f"{x}\t{count}\n")
  
with open("coverage_30x.txt", "w") as f:
